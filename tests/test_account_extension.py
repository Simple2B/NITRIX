...
def test_save_new(client):
    ...
    extensions = AccountExtension.query.filter(
        AccountExtension.account_id == ids["account_id"]
    ).all()
    assert response.status_code == 302
    assert extensions
    remove_data(ids)
...
def check_partial_data(client, blueprint):
    ...
    redirect = client.post(
        url_for(blueprint),
        data=dict(
            id=ids["account_id"], reseller_id=ids["reseller_id"], months=CORRECT_MONTHS
        ),
    )
    assert redirect.status_code == 302  # to the account.edit
    response = client.get(redirect.location)
...
def check_id_post(client, blueprint):
    ...
    redirect = client.post(url_for(blueprint), data=dict(id=EMPTY_ID))
    assert redirect.status_code == 302  # to the main.accounts
    response = client.get(redirect.location)
    assert VALIDATION_ERROR.encode() in response.data
    # what if id is not into request
    redirect = client.post(url_for(blueprint))
    assert redirect.status_code == 302  # to the main.accounts
    response = client.get(redirect.location)
    assert VALIDATION_ERROR.encode() in response.data
    # what if account_id isn't a number
    redirect = client.post(url_for(blueprint), data=dict(id=NOT_NUMBER_ID))
    assert redirect.status_code == 302  # to the main.accounts
    response = client.get(redirect.location)
    assert VALIDATION_ERROR.encode() in response.data
